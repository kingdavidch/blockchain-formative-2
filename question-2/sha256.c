...
void sha256_init(struct Sha256 *sha256, uint8_t hash[SIZE_OF_SHA256_HASH])
{
	sha256->hash = hash;
	sha256->chunk_pos = sha_256->chunk;
	sha256->space_left = SIZE_OF_SHA_256_CHUNK;
	sha256->total_len = 0;
	/*
...
void sha_256_write(struct Sha_256 *sha_256, const void *data, size_t len)
{
	...
	while (len > 0) {
		 ...
		 */
		if (sha_256->space_left == SIZE_OF_SHA256_CHUNK && len >= SIZE_OF_SHA256_CHUNK) {
			consume_chunk(sha256->h, p);
			len -= SIZE_OF_SHA256_CHUNK;
			p += SIZE_OF_SHA256_CHUNK;
			continue;
		}
...
void calc_sha_256(uint8_t hash[SIZE_OF_SHA256_HASH], const void *input, size_t len)
{
